...
| `bernstein tenant create\|grant\|revoke\|show\|verify\|slice\|showback\|verify-statement` | Chain-governed team boundaries. A tenant charter is not a config file the chain logs -- current charter state is the deterministic fold of `open` / `member_add` / `role_set` / `member_remove` / `quota_set` / `budget_set` / `close` events on the HMAC chain, so two verifiers reconstruct byte-identical state (and the same `charter_hash`) from the same segment. Every event carries `prev_event_hash`, giving charter history its own Merkle linkage inside the chain: appending a backdated event is refused outright, and editing a recorded one orphans its successor's hash *and* breaks `bernstein audit verify`, so `tenant verify` names the offending sequence number instead of silently folding to a different membership. `tenant slice` reuses the v2 multi-tenant export keyed on charter membership rather than on the free-form `tenant_id` string, so an event written by a principal the charter never enrolled stays out of the bundle (and is reported, not dropped) while the slice still verifies against the shared head. `tenant showback` emits a statement whose line items each carry their spend, task, lineage, and audit-chain links, bound by a per-item receipt hash, an order-sensitive item fold, and a statement hash -- amounts are integer nano-USD rounded exactly once at line-item creation, so aggregation order cannot change a digit; `tenant verify-statement` recomputes all of it from the statement file alone, with no ledger, chain, or network, and flipping any single field of any line item fails. Tenant certificates minted from a charter carry that charter's hash, so a run under a spawn-only certificate that tries to approve a gate is refused with a chain-anchored `tenant.duty_refusal` citing the certificate hash and the missing scope, and a certificate that *does* grant approve still refuses when the approver is the principal that spawned the work. See [docs/security/tenant-charters.md](docs/security/tenant-charters.md) and [docs/cost/showback-statement.md](docs/cost/showback-statement.md). |
