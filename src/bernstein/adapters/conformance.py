...
class StrategyDeclarationError(AssertionError):
    ...
    resume / dangerous-mode / event-channel / output-mode / session-state
    strategy. A missing row in
...
def assert_strategies_declared(adapter_names: list[str] | None = None) -> None:
    ...
    if missing:
        raise StrategyDeclarationError(
            ...
            f"(resume / dangerous-mode / event-channel / output-mode / session-state): {', '.join(missing)}"
        )
