...
class TestStaleMintIsRefusedNotWritten:
    """``record_charter_event`` re-reads the tail inside the append section.

    A caller that minted an event against a tail another writer has since
    moved must be refused rather than written: the log is append-only, so a
    duplicate ``seq`` could never be removed and the fold would stay
    unreadable forever. The refusal is the safety half of the design; the
    append section provides the liveness half (writers wait, not spin).
    """

    def test_an_event_minted_against_a_moved_tail_is_refused(self, tmp_path: Path) -> None:
        chain = AuditChainStore(tmp_path / "audit", key=KEY)
        opening = open_event(tenant_id="acme", principal="alice")
        record_charter_event(chain, opening)

        # Two writers read the same tail and each mint a successor.
        stale = next_event(
            opening, tenant_id="acme", kind=CHARTER_MEMBER_ADD, principal="alice", body={"principal": "bob"}
        )
        winner = next_event(
            opening, tenant_id="acme", kind=CHARTER_MEMBER_ADD, principal="alice", body={"principal": "carol"}
        )
        record_charter_event(chain, winner)

        with pytest.raises(CharterChainError) as excinfo:
            record_charter_event(chain, stale)
        assert excinfo.value.reason == "stale_predecessor"
        assert "Re-read the charter and mint again" in str(excinfo.value)

        # The refusal left nothing behind: the chain folds and the winner is
        # the tail.
        state = load_charter(chain, "acme")
        assert state.principals == frozenset({"alice", "carol"})

    def test_a_second_opening_is_refused_once_any_charter_exists(self, tmp_path: Path) -> None:
        chain = AuditChainStore(tmp_path / "audit", key=KEY)
        record_charter_event(chain, open_event(tenant_id="acme", principal="alice"))

        rival_opening = open_event(tenant_id="acme", principal="mallory")
        with pytest.raises(CharterChainError) as excinfo:
            record_charter_event(chain, rival_opening)
        assert excinfo.value.reason == "stale_predecessor"
        assert load_charter(chain, "acme").principals == frozenset({"alice"})
