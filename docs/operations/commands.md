...
| `bernstein tenant create\|grant\|revoke\|show\|verify\|slice\|showback\|verify-statement` | Chain-governed team boundaries. Current charter state is the deterministic fold of `open` / `member_add` / `role_set` / `member_remove` events on the HMAC chain, so two verifiers reconstruct byte-identical state (and the same `charter_hash`) from the same segment; every event carries `prev_event_hash`, so editing a recorded one orphans its successor's hash *and* breaks `bernstein audit verify`, and `tenant verify` names the offending sequence number. `tenant slice` keys the v2 multi-tenant export on charter membership rather than on the free-form `tenant_id` string, so an event written by a principal the charter never enrolled stays out of the bundle (and is reported, not dropped). `tenant showback` emits a statement whose integer nano-USD line items are bound by per-item receipt hashes, an order-sensitive fold, and a statement hash; `tenant verify-statement` recomputes all of it offline from the statement file alone. See [tenant charters](../security/tenant-charters.md) and [showback statements](../cost/showback-statement.md). |
